                ...
                <TableBody>
                  {filtered.map((c) => (
                    <TableRow key={c.id}>
                      ...
                      <TableCell>{c.totals ? (<div className="text-xs space-y-0.5"><div className="flex justify-between gap-4"><span className="text-muted-foreground">Impressões:</span><span className="font-medium">{c.totals.total_impressions.toLocaleString()}</span></div><div className="flex justify-between gap-4"><span className="text-muted-foreground">Cliques:</span><span className="font-medium">{c.totals.total_clicks.toLocaleString()}</span></div><div className="flex justify-between gap-4"><span className="text-muted-foreground">Conv. (Meta):</span><span className="font-medium text-green-600">{c.totals.total_leads}</span></div></div>) : (<span className="text-muted-foreground text-sm">Sem dados</span>)}</TableCell>
